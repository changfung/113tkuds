...
/*
 * 氣泡排序 Time Complexity: O(n) 最佳, O(n²) 最壞與平均
 * 說明：
 * 1. 已排序資料可在第一輪提早結束，最佳為 O(n)
 * 2. 最壞情況為完全反向排序，共需約 n(n-1)/2 次比較與交換 → O(n²)
 * 3. 平均情況下也需大量比較與交換 → O(n²)
 */

 /*
 * 雞尾酒排序 Time Complexity: O(n) 最佳, O(n²) 最壞與平均
 * 說明：
 * 1. 掃描方向為雙向，比單向氣泡排序略有效率
 * 2. 最佳情況一樣可提早結束 → O(n)
 * 3. 最壞與平均仍需完整左右掃描 → O(n²)
 */